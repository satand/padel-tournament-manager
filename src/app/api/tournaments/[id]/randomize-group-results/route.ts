...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { filled, skipped } = await randomizeMatches({
    tournamentId: id,
    rules,
    playersMap,
    mvpEnabled,
    through,
    matches: pending.map((m) => ({ id: m.id, participantAId: m.participantAId as string, participantBId: m.participantBId as string, phase: m.phase }))
  });
