...
export function RandomizeGroupResultsEasterEgg({ tournamentId, count }: Props) {
  ...
  return (
    <>
      <h2 onClick={onTitleClick} style={{ cursor: 'default' }}>Prossime partite ({count})</h2>
          ...
          <div onClick={(e) => e.stopPropagation()} className="panel" style={{ width: 'min(480px, 100%)', margin: 0 }}>
            ...
            {count === 0 ? (
              ...
              <p style={{ color: 'var(--muted)' }}>
                Vuoi chiudere con un <strong>punteggio generato a caso</strong> tutte le <strong>{count}</strong> partite di girone ancora senza risultato? Verrà stabilito un vincitore per ciascuna. La fase finale andrà generata a parte.
              </p>
            )}
