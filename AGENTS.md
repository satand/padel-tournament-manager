...
- Prisma: `npm run prisma:generate` (run after fresh install or schema changes, before dev/build), `npm run prisma:migrate` (migrate dev, dev-only).
...
- The app needs a reachable Postgres with migrations applied — there is **no demo/in-memory mode**: listing, creating and all write features (participants, calendar, results, exports) require the database.
